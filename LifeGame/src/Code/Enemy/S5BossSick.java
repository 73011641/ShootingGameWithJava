...
public class S5BossSick extends Boss {

    public S5BossSick(int x, int y, float delta_x, float delta_y, int max_x, int max_y, float delta_y_inc, Player aim) {
        ...
        for (int i = 1; i <= 3; ++i) {
            icon = new ImageIcon("src/Image/Enemy/S5BossSick" + i + ".png");
            img = icon.getImage();
            EnemyImg.add(img);
        }
        e_width = 320;
    ...
    public BossShot333 generateShotB3(int delta_y) { // 보스의 삼각함수
        BossShot333 Bshot = new BossShot333((int) x_pos - e_width / 2, (int) y_pos, delta_y, "src/Image/Shot/S5BossShot1.png");
        return Bshot;
    }

    //한발 두발 세발
    public BossShotAdd generateShotBA(int delta_y) { // 보스의 애드 샷
        BossShotAdd Bshot2 = new BossShotAdd((int) x_pos - e_width / 2, (int) y_pos, delta_y, "src/Image/Shot/S5BossShot1.png");
        return Bshot2;
    }
}
